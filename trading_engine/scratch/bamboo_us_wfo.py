"""
trading_engine/scratch/bamboo_us_wfo.py
Backtest + Walk-Forward validation for Bamboo US stocks via yfinance.
Validates all 10 tickers added to default_bamboo_us_assets.
"""
...
BAMBOO_US = ["AAPL","MSFT","NVDA","AMZN","GOOGL","META","TSLA","JPM","NFLX","AMD"]
TRAIN_DAYS=300; TEST_DAYS=100; STOP_LOSS_PCT=0.05; RISK_PCT=0.02; INITIAL_CAPITAL=10_000

def fetch_yf(ticker, days=600):
    period="3y" if days>=600 else "2y"
    df=yf.download(ticker,period=period,interval="1d",progress=False,auto_adjust=True)
    if df is None or df.empty: raise RuntimeError(f"No yf data for {ticker}")
    df.columns=[c.lower() if isinstance(c,str) else c[0].lower() for c in df.columns]
    return df[["open","high","low","close","volume"]].tail(days).astype(float)
...
def backtest_signals(df, buy_sig, sell_sig):
    ...
    for i in range(1,len(df)):
        ...
        if in_trade:
            ...
            hit_stop=lows[i]<=trailing_stop; hit_tp=highs[i]>=entry_price*1.25; hit_sell=sells[i]
            if hit_stop or hit_tp or hit_sell:
                exit_p=trailing_stop if hit_stop else (entry_price*1.25 if hit_tp else price)
...
def strategy_ema_cross(df):
    ...
    buy=((ema10>ema30)&(ema10.shift(1)<=ema30.shift(1))&(rsi>=40)&(rsi<=70)&(vol>=avg_vol*0.8)).fillna(False)
    sell=(((ema10<ema30)&(ema10.shift(1)>=ema30.shift(1)))|(rsi>78)).fillna(False)
    return buy,sell

def strategy_mom_breakout(df):
    close,vol=df["close"],df["volume"]; avg_vol=vol.rolling(20).mean()
    buy=((close>close.rolling(20).max().shift(1))&(vol>avg_vol*1.3)).fillna(False)
...
def run_wfo(df):
    total=len(df)
    if total<TRAIN_DAYS+TEST_DAYS: return None,f"Only {total} rows"
    ...
    while start+TRAIN_DAYS+TEST_DAYS<=total and fold_idx<3:
        ...
        folds.append({"fold":fold_idx,"strategy":best_strat,"oos_result":r_oos})
        start+=TEST_DAYS; fold_idx+=1
    return folds,"OK"

print("="*72)
print(f"{'BAMBOO US STOCKS WALK-FORWARD VALIDATION':^72}")
print(f"{'via yfinance | Train:300d | Test:100d | 3 folds':^72}")
print("="*72)

summary=[]
for ticker in BAMBOO_US:
    try:
        df=fetch_yf(ticker,600)
        # Full in-sample backtest
        r_ema=backtest_signals(df,*strategy_ema_cross(df)); r_mom=backtest_signals(df,*strategy_mom_breakout(df))
        best_r,best_name = (r_ema,"EMA_Cross") if composite_score(r_ema)>=composite_score(r_mom) else (r_mom,"MomBreakout")
        # WFO
        folds,msg=run_wfo(df)
        if folds is None:
            print(f"  {ticker:6s}: SKIP — {msg}"); summary.append({"ticker":ticker,"status":"skip"}); continue
        ...
        passes=avg_wr>=45.0 and avg_pf>=1.0 and oos_trades>=2
        flag="✅" if passes else "❌"
        print(f"  {ticker:6s}: {flag} {'PASS' if passes else 'DEMOTE'} | IS_ret={best_r['return_pct']:+6.2f}% [{best_name}]"
              f" | OOS trades={oos_trades} avg_WR={avg_wr:.1f}% avg_PF={avg_pf:.2f}")
        summary.append({"ticker":ticker,"status":"pass" if passes else "demote",
                        "is_strategy":best_name,"is_result":best_r,
                        "oos_trades":oos_trades,"avg_wr":round(avg_wr,1),"avg_pf":round(avg_pf,2)})
    except Exception as e:
        print(f"  {ticker:6s}: ERROR — {e}"); summary.append({"ticker":ticker,"status":"error","reason":str(e)})

print(); print("="*72)
passes_list=[s["ticker"] for s in summary if s["status"]=="pass"]
demotes=[s["ticker"] for s in summary if s["status"]=="demote"]
errors=[s["ticker"] for s in summary if s["status"] in ("error","skip")]
print(f"  PASS   ({len(passes_list)}): {passes_list}")
print(f"  DEMOTE ({len(demotes)}): {demotes}")
print(f"  SKIP/ERROR ({len(errors)}): {errors}")

out_path=PROJECT_ROOT/"trading_engine"/"backtest_results"/"bamboo_us_wfo.json"
