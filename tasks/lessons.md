...
2. **Reconciliation Cost Basis Accuracy**:
   - Always verify that `historical_costs` reflects the exact verified purchase price from Bybit (`fetch_my_trades`) so the dashboard reconciler does not fall back to synthetic percentage gaps.
3. **Hard Profit Floor**:
